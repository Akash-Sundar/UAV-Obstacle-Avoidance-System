...
import sys
sys.path.append('/home/akash/MEAM620/proj1_3/meam620/')
...
def calculate_metrics(results, world, goal):
    ...
    metrics = {}
    metrics['stopped_at_goal'] = bool((results['exit'] == ExitStatus.COMPLETE) and np.linalg.norm(results['state']['x'][-1] - goal) <= 0.05)
